...
public Action:Command_SetMyBoss(client, args)
{
	...
	for (new i = 0; (xBossKV=FF2_GetSpecialKV(i,true)); i++)
	{
		...
		KvGetString(xBossKV, "name", Special_Name, 64);
...
public Action:OnPlayerDeath(Handle:event, const String:eventName[], bool:dontBroadcast)
{
	...
	if(!IsBoss(client))
	{
		if(!(GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER))
		{
			...
			if(!attacker || attacker == client && DeadRunMode)
			{
				for(new i=1; i<=MaxClients; i++)
				{
					new activator=FF2_GetBossIndex(i); 
					if(activator!=-1)
						SetEventInt(event, "attacker", activator);
				}
			}
		}
