...
new bool:IsNextBoss[MAXPLAYERS+1]=false;

// Config file paths
new String:bLog[256], String:eLog[256];
...
float PrepareMercAt[MAXPLAYERS+1];
float InspectPlayerInventoryAt[MAXPLAYERS+1];
float KillRPSLosingBossAt[MAXPLAYERS+1];
...
new configVersionMode[MaxBosses];
...
public LoadCharacter(const String:character[])
{
    ...
    if(!FileExists(config))
    {
        LogToFile(eLog,"[FF2] Character %s does not exist!", character);
        return;
    }
    ...
    for(new i=1; ; i++)
    {
        ...
        if(KvJumpToKey(BossKV[Specials], config))
        {
            ...
            if(!FileExists(config))
            {
                LogToFile(bLog, "[FF2 Bosses] Character %s needs plugin %s!", character, plugin_name);
                return;
            }
        }
        else
    ...
    configVersionMode[Specials]=KvGetNum(BossKV[Specials], "version", 1);
            ...
            for(new i=1; ; i++)
            {
                ...
                if(bSkipFileChecks[Specials])
                {
                    if(!FileExists(config, true))
                        LogToFile(bLog, "[FF2 Bosses] Character '%s' will be skipping file checks for '%s'", character, config);
                ...
                else
                {
                    if(FileExists(config, true))
                        ...
                        LogToFile(bLog, "[FF2 Bosses] Character %s is missing file '%s'!", character, config);
                }
                #else
                AddFileToDownloadsTable(config);
                #endif
                ...
                for(new extension; extension<sizeof(extensions); extension++)
                {
                    ...
                    if(bSkipFileChecks[Specials])
                    {
                        if(!FileExists(key, true) && StrContains(key, ".phy")==-1)
                            LogToFile(bLog, "[FF2 Bosses] Character '%s' will be skipping file checks for '%s'", character, key);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                LogToFile(bLog, "[FF2 Bosses] Character %s is missing file '%s'!", character, key);
                            }
                        }
                    }
                    #else
...
public Action:Event_Setup(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(configVersionMode[Special[0]]==1)
    {
        KvRewind(BossKV[Special[0]]);
        new defaultRageDamage=GetConVarInt(cvarDefaultRageDamage);
        BossRageDamage[0]=KvGetNum(BossKV[Special[0]], "ragedamage", defaultRageDamage);
        if(BossRageDamage[0]<=0)
        {
            decl String:bossName[64];
            KvGetString(BossKV[Special[0]], "name", bossName, sizeof(bossName));
            PrintToServer("[FF2 Bosses] Warning: Boss %s's rage damage is 0 or below, setting to %i", bossName, defaultRageDamage);
            BossRageDamage[0]=defaultRageDamage;
        }
    ...
    for(new client=1;client<=MaxClients;client++)
    {
        ...
        else if (BossCookieSetting[client] == FF2Setting_Unknown || !BossCookieSetting[client])
        {
            decl String:nick[64]; 
            GetClientName(client, nick, sizeof(nick));
...
public Action:Timer_MusicPlay(Handle:timer, any:client)
{
    ...
    if(KvJumpToKey(BossKV[Special[0]], "sound_bgm"))
    {
        ...
        {
            MusicIndex++;
            Format(music, sizeof(music), "time%i", MusicIndex);
        }
        ...
        if(strlen(music[0])>5)
        {
            if(!client)
            {
                EmitSoundToAllExcept(NoMusic, music);
            }
            else if(CheckSoundException(client, NoMusic))
            {
    ...
    if(Enabled && CheckRoundState()==FF2RoundState_RoundRunning)
    {
        ...
        if(KvJumpToKey(BossKV[Special[0]], "sound_bgm"))
        {
            ...
            if(strlen(music[0])>5)
            {
                if(!client)
                {
                    EmitSoundToAllExcept(NoMusic, music, _, _, _, _, _, _, _, _, _, false);
                }
                else if(CheckSoundException(client, NoMusic))
                {
                    EmitSoundToClient(client, music);
...
stock SetInfoCookies(client,infonum,value)
{
    ...
    GetClientCookie(client, FF2Cookies, s, 24);
    ExplodeString(s, " ", ff2cookies_values,8,5);
    Format(s,sizeof(s),"%s %s %s %s",ff2cookies_values[0],ff2cookies_values[1],ff2cookies_values[2],ff2cookies_values[3]);
    for(new i;i<infonum;i++)
        Format(s,sizeof(s),"%s %s",s,ff2cookies_values[4+i]);
...
SetClientSoundOptions(client, excepttype, bool:on)
{
    ...
    Format(s, sizeof(s), "%s %s %s %s %s %s %s %s", ff2cookies_values[0], ff2cookies_values[1], ff2cookies_values[2], ff2cookies_values[3], ff2cookies_values[4], ff2cookies_values[5], ff2cookies_values[6], ff2cookies_values[7]);
    SetClientCookie(client, FF2Cookies, s);
}
...
public Action:Command_SetMyBoss(client, args)
{
    ...
    SetMenuTitle(dMenu, "%t\n%t\n%t","ff2_boss_selection", xIncoming[client][0]=='\0' ? "None" : xIncoming[client], (BossCookieSetting[client]==FF2Setting_Disabled ? "ff2boss_disabled" : "ff2boss_enabled"), (CompanionCookieSetting[client]==FF2Setting_Disabled ? "ff2comp_disabled" : "ff2comp_enabled"));
...
public Action:MakeModelTimer(Handle:timer, any:client)
{
    if(IsValidClient(Boss[client], true) && CheckRoundState()!=FF2RoundState_RoundEnd)
    {
        ...
        KvRewind(BossKV[Special[client]]);
        #if defined FILECHECK_ENABLED
        KvGetString(BossKV[Special[client]], "filename", bossName, sizeof(bossName));
        #endif
        KvGetString(BossKV[Special[client]], "model", model, sizeof(model));
        #if defined FILECHECK_ENABLED
        if(bSkipFileChecks[Special[client]])
        {
            if(!FileExists(model))
...
EquipBoss(boss)
{
    ...
    for(new i=1; ; i++)
    {
        ...
        if(KvJumpToKey(BossKV[Special[boss]], weapon))
        {
            ...
            new wepIdx=KvGetNum(BossKV[Special[boss]], "index");
            new BossWeapon=SpawnWeapon(client, weapon, wepIdx, 101, 5, attributes, bool:KvGetNum(BossKV[Special[boss]], "show", 0));
            ...
            if(!KvGetNum(BossKV[Special[boss]], "show", 0))
            {
                ...
                if(wepIdx==221 || wepIdx==572 || wepIdx==999 || wepIdx==1013) // Workaround for jiggleboned weapons
                {
            ...
            new wepA=KvGetNum(BossKV[Special[boss]], "alpha", 255);
            new wepR=KvGetNum(BossKV[Special[boss]], "red", 255);
            new wepG=KvGetNum(BossKV[Special[boss]], "green", 255);
            new wepB=KvGetNum(BossKV[Special[boss]], "blue", 255);
            if(wepA<255 || wepR<255 || wepG<255 || wepB<255)
            {
                SetEntityRenderMode(BossWeapon, RENDER_TRANSCOLOR);
                SetEntityRenderColor(BossWeapon, wepR, wepG, wepB, wepA);
            }
...
public Action:MakeBoss(Handle:timer, any:boss)
{
    ...
    if(KvGetNum(BossKV[Special[boss]], "version")==1 && KvGetNum(BossKV[Special[boss]], "subversion")>=31 || KvGetNum(BossKV[Special[boss]], "version")>=2)
    {
        dmgTriple[client]=false;
    }
...
public OnClientCookiesCached(client)
{
    ...
    {
        BossCookieSetting[client] = FF2Prefs:enabled;
    }
    ...
    {
        CompanionCookieSetting[client] = FF2Prefs:enabled;
    }
}
...
public OnClientDisconnect(client)
{
    if(Enabled)
    {
        ...
        if(IsValidClient(client, true) && CheckRoundState()==FF2RoundState_RoundRunning)
        {
...
public FF2_Tick(Float:gameTime)
{
    ...
    {    
        ...
        Call_PushCell(AliveBossPlayers+AliveMinionPlayers);
        Call_Finish();
    ...
    {    
        ...
        if(Enabled2)
        {
            switch(announcecount)
            {
                case 1:
                {
                    CPrintToChatAll("=== {blue}Big Bang Gamers{default} Freak Fortress 2 v%s (a fork by SHADoW NiNE TR3S) ===", PLUGIN_VERSION);
                }
                case 2:
                {
    ...
    if(gameTime >= DrawGameTimerAt)
    {
        ...
        SetHudTextParams(-1.0, 0.17, 1.1, time<=(countdownTime/2) ? 255 : 0, time>(countdownTime/4) ? 255 : 0, 0, 255);
        for(new client; client<=MaxClients; client++)
        {
            if(IsValidClient(client))
            {
    ...
    if(gameTime >= StartBossAt)
    {
        ...
        for(new boss; boss<=MaxClients; boss++)
        {
            ...
            {
                if(configVersionMode[Boss[boss]]>1)
                {
                    BossHealthMax[boss]=ParseFormulaV2(boss, "health", RoundFloat(Pow((760.8+float(playing))*(float(playing)-1.0), 1.0341)+2046.0));
                    BossLivesMax[boss]=BossLives[boss]=ParseFormulaV2(boss, "lives", 1);
                    BossHealth[boss]=BossHealthLast[boss]=BossHealthMax[boss]*BossLivesMax[boss];
                    BossRageDamage[boss]=ParseFormulaV2(boss, "rage_damage", GetConVarInt(cvarDefaultRageDamage));
                    BossSpeed[boss]=float(ParseFormulaV2(boss, "speed", GetConVarInt(cvarDefaultMoveSpeed)));
                }
                else
                {
...
public FF2_RoundTick(Float:gameTime)
{
    if(gameTime >= FF2BossTick)
    {
        ...
        for(int client; client<=MaxClients; client++)
        {
            ...
            new String:lives[MaxAbilities][3];
            for(new i=1; ; i++)
            {
...
UseRage(client) // Activating our RAGE ability here
{
    ...
    if(RoundFloat(BossCharge[boss][0])==100)
    {
        ...
        char ability[10], lives[MaxAbilities][3];
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    for(new lives=1; lives<BossLives[boss]; lives++)
    {
        ...
        {
            SetEntityHealth(client, (BossHealth[boss]-damage)-BossHealthMax[boss]*(lives-1));  //Set the health early to avoid the boss dying from fire, etc.  //Set the health early to avoid the boss dying from fire, etc.
            ...
            for(new n=1; n<MaxAbilities; n++)
            {
                ...
                if(KvJumpToKey(BossKV[Special[boss]], ability))
                {
                    ...
                    {
                        new String:stringLives[MaxAbilities][3];
            ...
            for(new target=1; target<=MaxClients; target++)
            {
                ...
                {
                    ShowGameText("%t", ability, bossName, BossLives[boss]);
                }
            }
    ...
    for(new target; target<=MaxClients; target++)
    {
        if(IsValidClient(target, true) && (GetHealingTarget(target, true)==attacker))
        {
    ...
    {
        if(IsValidClient(healers[target], true))
        {
    ...
    if(IsValidClient(attacker))
    {
        ...
        if(!IsBoss(attacker) && IsValidEntity(weapon))
        {
            static kStreakCount;
            kStreakCount+=damage;
            if(kStreakCount>=GetConVarInt(cvarDmg2KStreak))
            {
...
public Action:OnTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
    ...
    else
    {
        ...
        if(boss!=-1)
        {
            if(attacker<=MaxClients)
            {
                ...
                if(bIsTelefrag)
                {
                    ...
                    damage=(BossHealth[boss]>9001 ? 9001.0 : float(GetClientHealth(Boss[boss]))+90.0);
                ...
                switch(index)
                {
                    ...
                    case 593:  //Third Degree
                    {
                        ...
                        {
                            if(IsValidClient(healer, true) && (GetHealingTarget(healer, true)==attacker))
                            {
                        ...
                        {
                            if(IsValidClient(healers[healer], true))
                            {
        ...
        else
        {
            ...
            if(IsValidClient(client) && TF2_GetPlayerClass(client)==TFClass_Soldier)  //TODO: LOOK AT THIS
            {
                if(damagetype & DMG_FALL)
                {
...
stock ParseFormulaV2(boss, const String:key[], defaultValue)
{
    decl String:formula[1024], String:bossName[64];
    KvRewind(BossKV[Special[boss]]);
    KvGetString(BossKV[Special[boss]], "name", bossName, sizeof(bossName), "=Failed name=");
    KvGetString(BossKV[Special[boss]], key, formula, sizeof(formula));
    if(!formula[0])
    {
        return defaultValue;
    }
    ...
    {
        return RoundFloat(result/GetConVarFloat(cvarMedievalDivider));
    }
    return result;
}
...
FindCompanion(boss, players, bool:omit[])
{
    ...
    if(playersNeeded<players && strlen(companionName))  //Only continue if we have enough players and if the boss has a companion
    {
        ...
        {
            if(configVersionMode[Special[companion]]==1)
            {
                KvRewind(BossKV[Special[companion]]);
                new defaultRageDamage=GetConVarInt(cvarDefaultRageDamage);
                BossRageDamage[companion]=KvGetNum(BossKV[Special[companion]], "ragedamage", defaultRageDamage);
                if(BossRageDamage[companion]<=0)
                {
                    new String:bossName[64];
                    KvGetString(BossKV[Special[companion]], "name", bossName, sizeof(bossName));
                    LogToFile(bLog, "[FF2 Bosses] Warning: Boss %s's rage damage is below 0, setting to %i", bossName, defaultRageDamage);
                    BossRageDamage[companion]=defaultRageDamage;
                }
            
                BossLivesMax[companion]=KvGetNum(BossKV[Special[companion]], "lives", 1);
                if(BossLivesMax[companion]<=0)
                {
                    LogToFile(bLog, "[FF2 Bosses] Warning: Boss %s has an invalid amount of lives, setting to 1", companionName);
...
public MusicTogglePanelH(Handle:menu, MenuAction:action, param1, param2)
{
    if(IsValidClient(param1))
    {
        if(action==MenuAction_Select)
        {
            if(param2==2)
            {
                SetClientSoundOptions(param1, NoMusic, false);
                KvRewind(BossKV[Special[0]]);
                if(KvJumpToKey(BossKV[Special[0]],"sound_bgm"))
                {
                    new String:s[PLATFORM_MAX_PATH];
                    Format(s,sizeof(s),"path%i",MusicIndex);
                    KvGetString(BossKV[Special[0]], s,s, sizeof(s));
                    StopSound(param1, SNDCHAN_AUTO, s);
                    StopSound(param1, SNDCHAN_AUTO, s);
                }
            }
            else
                SetClientSoundOptions(param1, NoMusic, true);
...
public Native_GetRageDist(Handle:plugin, numParams)
{
    ...
    if(!ability_name[0])
    {
        return _:KvGetFloat(BossKV[Special[index]],"ragedist",GetConVarFloat(cvarDefaultRageDist));
    }
    new String:s[10];
    for(new i=1; i<MaxAbilities; i++)
    {
        ...
        if(KvJumpToKey(BossKV[Special[index]],s))
        {
            ...
            {
                KvRewind(BossKV[Special[index]]);
                see=KvGetFloat(BossKV[Special[index]],"ragedist",GetConVarFloat(cvarDefaultRageDist));
            }
            return _:see;
        }
    }
...
public Native_HasAbility(Handle:plugin, numParams)
{
    ...
    {
        LogToFile(eLog,"Failed KV: %i %i", boss, Special[boss]);
        return false;
    }
...
public Native_GetSpecialKV(Handle:plugin, numParams)
{
    ...
    {
        if(index!=-1 && index<=MaxClients && Special[index]!=-1 && Special[index]<MaxBosses)
        {
...
{
    return AliveBossPlayers+AliveMinionPlayers;
}
