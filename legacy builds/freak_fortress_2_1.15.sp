...
// Bools
bool smac=false,
	 emitRageSound[MAXPLAYERS+1],
	 Enabled=true,
	 Enabled2=true,
	 BossCrits=true,
	 SpecForceBoss,
	 lastPlayerGlow=true,
	 bossTeleportation=true,
	 canBossRTD,
	 checkDoors=false,
	 bMedieval,
	 firstBlood,
	 ...
	 DeadRunMode = false,
	 IsPreparing = false,
	 IsCompanion[MAXPLAYERS+1],
	 areSubPluginsEnabled;
...
int OtherTeam=2,
	...
	BlueAlivePlayers,
	RoundCount,
	Special[MAXPLAYERS+1],
	Incoming[MAXPLAYERS+1],
	MusicIndex,
	Damage[MAXPLAYERS+1],
	curHelp[MAXPLAYERS+1],
	uberTarget[MAXPLAYERS+1],
	shield[MAXPLAYERS+1],
	detonations[MAXPLAYERS+1],
	FF2flags[MAXPLAYERS+1],
	Boss[MAXPLAYERS+1],
	BossHealthMax[MAXPLAYERS+1],
	BossHealth[MAXPLAYERS+1],
	BossHealthLast[MAXPLAYERS+1],
	BossLives[MAXPLAYERS+1],
	BossLivesMax[MAXPLAYERS+1],
	BossRageDamage[MAXPLAYERS+1],
	KSpreeCount[MAXPLAYERS+1],
	shortname[MAXPLAYERS+1],
	timeleft,
	PointDelay=6,
	AliveToEnable=5,
	PointType,
...
float BossCharge[MAXPLAYERS+1][8],
	  Stabbed[MAXPLAYERS+1],
	  Marketed[MAXPLAYERS+1],
	  KSpreeTimer[MAXPLAYERS+1],
	  GlowTimer[MAXPLAYERS+1],
	  Announce=120.0,
	  circuitStun,
	  GoombaDamage=0.05,
	  reboundPower=300.0,
...
void OnPluginStart()
{
	LogMessage("===Initializing Freak Fortress 2 -v%s brought to you by BigBangGamers===", PLUGIN_VERSION);
	...
	HookEvent("teamplay_round_start", OnSetupTime);
	HookEvent("arena_round_start", OnRoundStart);
	HookEvent("arena_win_panel", OnRoundEnd);
	HookEvent("player_spawn", OnPlayerSpawn, EventHookMode_Pre);
	HookEvent("player_death", OnPlayerDeath, EventHookMode_Pre);
	HookEvent("player_chargedeployed", OnUberDeploy);
	HookEvent("player_hurt", OnPlayerHurt, EventHookMode_Pre);
	HookEvent("object_destroyed", OnDestroy, EventHookMode_Pre);
...
public void OnMapStart()
{
	...
	char checkdr[1024];
	GetCurrentMap(checkdr, 1024);
	if (!StrContains(checkdr,"vsh_dr_") || !StrContains(checkdr,"dr_") || !StrContains(checkdr,"deadrun_"))
	{
		DeadRunMode = true;
...
public void OnMapEnd()
{
	if(Enabled || Enabled2)
	{
		...
		if(smac && FindPluginByFile("smac_cvars.smx")!=null)
		{
...
{
	char file[PLATFORM_MAX_PATH], key[PLATFORM_MAX_PATH], section[64];
...
stock bool: IsTeamIndependent()
{
	char map[PLATFORM_MAX_PATH];
	GetCurrentMap(map, PLATFORM_MAX_PATH);
	if(!StrContains(map, "zf_")) return false;
	if(!StrContains(map, "dr_")) return false;
	if(!StrContains(map, "vsh_dr_")) return false;
	if(!StrContains(map, "deathrun_")) return false;
	if(!StrContains(map, "vsh_citypeak")) return false;
	if(!StrContains(map, "vsh_2fortdesk")) return false;
	if(!StrContains(map, "vsh_aperture")) return false;
	if(!StrContains(map, "vsh_oilrig")) return false;
	if(!StrContains(map, "vsh_manncohq")) return false;
	if(!StrContains(map, "vsh_top_arena")) return false;
	return true;
...
public Action:OnSetupTime(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if(IsValidClient(player) && !IsPlayerAlive(player) && GetClientTeam(player)>1)
			ChangeClientTeam(player, GetRandomInt(2,3));
	}
	...
	switch(KvGetNum(BossKV[Special[0]], "bossteam", 0))  //Check if the boss sponsors a specific team or not
	{
		case 1: // Bosses that don't sponsor a team at all
		{
			RandomBossTeam();
		}
		case 2: // RED Boss
		...
		default: // Use CVAR
		{
			...
			{
				case 1: // Boss team is random
				{
					RandomBossTeam();
				}
				case 2: // Boss team is RED
				{
					REDBossTeam();
				}
				case 3: // Boss team is BLU
				{
					BLUBossTeam();
				}
				default: // Use FF2 Logic
				{
					if(!StrContains(currentmap, "vsh_") || !IsTeamIndependent())
					{
						BLUBossTeam();
					}
					else
	...
	for(new client; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && (GetClientTeam(client) == BossTeam || GetClientTeam(client) == OtherTeam))
		{
			if(!IsBoss(client) && GetClientTeam(client)==BossTeam)
			{
				AssignTeam(client, OtherTeam);
				TF2Attrib_RemoveByDefIndex(client, 259);
				TF2Attrib_RemoveByDefIndex(client, 68);
			}
			if(IsBoss(client) && GetClientTeam(client)==OtherTeam)
			{
...
public Action:OnRoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	char text[128], sound[PLATFORM_MAX_PATH];
	bool bossWin=false;
	executed=false;
	executed2=false;
	...
	for(new client; client<=MaxClients; client++)
	{
		...
		{
			if(BossInfoTimer[client][timer]!=null)
			{
				KillTimer(BossInfoTimer[client][timer]);
				BossInfoTimer[client][timer]=null;
			}
		}
	}
...
AssignTeam(client, team)
{
	...
	if(IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		SetEntProp(client, Prop_Send, "m_lifeState", 0);
		TF2_RespawnPlayer(client);
		if((!StrContains(map, "vsh_") || !StrContains(map, "zf_") || !StrContains(map, "dr_")) && BossTeam == 2 && OtherTeam == 3)
		{
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	switch(iItemDefinitionIndex)
	{
		...
		{
			Handle itemOverride=PrepareItemHandle(item, _, _, "25 ; 0.5 ; 207 ; 1.33 ; 144 ; 1.0 ; 58 ; 3.2", true);
			if(itemOverride!=null)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		{
			Handle itemOverride=PrepareItemHandle(item, _, _, "10 ; 1.25 ; 178 ; 0.75 ; 144 ; 2.0 ; 11 ; 1.5");
		...
		case 220:  //Shortstop
		{
			Handle itemOverride=PrepareItemHandle(item, _, _, "328 ; 1.0", true);
			if(itemOverride!=null)
			{
				item=itemOverride;
				return Plugin_Changed;
...
stock Handle:PrepareItemHandle(Handle:item, String:name[]="", index=-1, const String:att[]="", bool:dontPreserve=false)
{
	...
	//Handle weapon=TF2Items_CreateItem(flags);  //null;  Going to uncomment this since this is what Randomizer does
...
public Action:CheckItems(Handle:timer, any:client)
{
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			...
			case 1101: // BASE Jumper
			{
				if(DeadRunMode)
				{
					TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
					SpawnWeapon(client, "tf_weapon_grenadelauncher", 19, 1, 0, "");	
				}		
			}
		}
	}
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			...
			case 20, 130, 207, 661, 797, 806, 886, 895, 904, 913, 962, 971, 1150: // All Sticky Launchers
			{
				if(DeadRunMode)
				{
					TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
			...
			case 1101: // BASE Jumper
			{
				...
				{
					TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
					SpawnWeapon(client, "tf_weapon_shotgun_soldier", 10, 1, 0, "");	
				}		
			}
		}
	}
...
public Action OnDestroy(Event event, const char[] name, bool dontBroadcast)
{
	if(Enabled)
	{
...
public Action:ClientTimer(Handle:timer)
{
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			bool validwep=!strncmp(classname, "tf_wea", 6, false);
...
public Action OnPlayerDeath(Event event, const char[] eventname, bool dontBroadcast)
{
	...
	if(!IsBoss(client))
	{
		...
		if(IsBoss(attacker))
		{
			...
			if(GetEventInt(event, "customkill") & TF_CUSTOM_BOOTS_STOMP) // Prevents gaining extra HP from Mantreads Stomping players
			{					
				if(stompdmg < FF2_GetBossHealth(boss))
					FF2_SetBossHealth(boss, FF2_GetBossHealth(boss)-RoundToNearest(stompdmg));
			}
		}
	}
	else
...
public Action:PlaySoundKill(Handle:timer, Handle:data)
{
	...
	if(client)
	{
		...
		char class[32], sound[PLATFORM_MAX_PATH];
...
public Action OnRocketJump(Event event, const char[] name, bool dontBroadcast)
{
	if(Enabled)
	{
		if(StrEqual(name, "rocket_jump", false))
		{
			FF2flags[GetClientOfUserId(event.GetInt("userid"))]|=FF2FLAG_ROCKET_JUMPING;
		}
		else
		{
			FF2flags[GetClientOfUserId(event.GetInt("userid"))]&=~FF2FLAG_ROCKET_JUMPING;
...
public Action OnPlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		if((BossHealth[boss]-damage<=BossHealthMax[boss]*lives) && CheckRoundState()==1)
		{
...
public Action:OnTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if((attacker<=0 || client==attacker) && IsBoss(client))
	{
		...
		{
			new IsABoss = FF2_GetBossIndex(client);
			if (IsABoss!=-1)
			{
				stompdmg = damage;
				if(damagetype & DMG_FALL)
				{
					if(stompdmg >= FF2_GetBossHealth(IsABoss)+1)
						return Plugin_Handled;
					else
					{
						FF2_SetBossHealth(IsABoss, FF2_GetBossHealth(IsABoss)+RoundToNearest(stompdmg));
						return Plugin_Continue;
					}
				}
			}
...
{
	char formula[1024], buffer[2], value[1024];
	bool mustClose, usePlayers, canAdd, valueReady;
	new parentheses;
	float sum[128];
	new _operator[128];

	KvRewind(BossKV[Special[client]]);
	KvGetString(BossKV[Special[client]], "health_formula", formula, sizeof(formula), "((760.8+n)*n)^1.04");
	...
	if(parentheses || health<=0)
	{
		...
		health=RoundFloat(Pow(((760.8+playing)*playing), 1.0341)) + 2046;
	}
...
FindCompanion(boss, players, bool:omit[])
{
	...
	if(playersNeeded<players && strlen(companionName))  //Only continue if we have enough players and if the boss has a companion
	{
		...
		IsCompanion[Boss[companion]]=true;
...
public Action:NewPanel(client, versionIndex)
{
	...
	SetGlobalTransTarget(client);
	Format(whatsNew, 9000, "=%t:=", "whatsnew", ff2versiontitles[versionIndex], ff2versiondates[versionIndex]);
	SetPanelTitle(panel, whatsNew);
	FindVersionData(panel, versionIndex);
	if(versionIndex>0)
	{
		Format(whatsNew, 9000, "%t", "older");
	}
	else
	{
		Format(whatsNew, 9000, "%t", "noolder");
	}
...
public ClassinfoTogglePanelH(Handle:menu, MenuAction:action, param1, param2)
{
	if(IsValidClient(param1))
	{
		if(action==MenuAction_Select)
		{
			...
			ExplodeString(s, " ", ff2cookies_values,8,5);
			if(param2==2)
				Format(s,9000,"%s %s %s 0 %s %s %s",ff2cookies_values[0],ff2cookies_values[1],ff2cookies_values[2],ff2cookies_values[4],ff2cookies_values[5],ff2cookies_values[6],ff2cookies_values[7]);
			else
				Format(s,9000,"%s %s %s 1 %s %s %s",ff2cookies_values[0],ff2cookies_values[1],ff2cookies_values[2],ff2cookies_values[4],ff2cookies_values[5],ff2cookies_values[6],ff2cookies_values[7]);
...
public Action:Timer_DisplayCharsetVote(Handle:timer)
{
	...
	Handle menu=CreateMenu(NextmapPanelH, MenuAction:MENU_ACTIONS_ALL);
...
{
	Timer_MusicPlay(null,GetNativeCell(1));
}
