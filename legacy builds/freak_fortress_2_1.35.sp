...
public LoadCharacter(const String:character[])
{
	...
	if(version!=1)
	{
		LogToFile(bLog, "[FF2 Bosses] Character %s is only compatible with FF2 v%i!", character, version);
		return;
	}
...
{
	if(!GetEntProp(client, Prop_Send, "m_iObserverMode")) return false;
...
public Action:MakeBoss(Handle:timer, any:boss)
{
	...
	{
		AssignTeam(client, TFTeam:BossTeam);
	}
...
public Action:Command_MakeNextBoss(client, args)
{
	...
	if(args!=1)
	{
		if(args==1 && IsValidClient(client))
		{
			...
			CPrintToChatAll("{olive}[FF2]{default} %s will become the boss next round!", client);
			Command_YouAreNext(client, 0);
			return Plugin_Handled;
		}
		else
...
public void Timers_PreThink(int client, float gTime)
{
	...
	if(gTime>=KillRPSLosingBossAt[client])
	{
		if(IsPlayerAlive(client))
		{
			SDKHooks_TakeDamage(client, RPSWinner, RPSWinner, float(BossHealth[client]), DMG_GENERIC, -1);
		}
		KillRPSLosingBossAt[client]=INACTIVE;
	}

	if(gTime>=skewerCooldownEndsIn[client])
	{
	...
	if(!IsBoss(client) && condition==TFCond_Taunting)
	{
		...
		if(IsValidEdict(weapon))
		{
			...
			if(StrEqual(classname, "tf_weapon_compound_bow", false))
			{
				...
				skewerCooldownEndsIn[client]=GetEngineTime()+6.0;
				return;
			}
...
public void Frame_RemoveMeleeRestrict(int client)
{
	if (IsClientInGame(client))
	{
	...
	if(playersNeeded<players && strlen(companionName))  //Only continue if we have enough players and if the boss has a companion
	{
		...
		if(PickCharacter(boss, companion))  //TODO: This is a bit misleading
		{
			...
			if(BossRageDamage[companion]<=0)
			{
				...
				LogToFile(bLog, "[FF2 Bosses] Warning: Boss %s's rage damage is below 0, setting to 1900", bossName);
