...
enum
{
	FF2RoundState_PreGame = -1,
	FF2RoundState_PreRound,
	FF2RoundState_RoundActive,
	FF2RoundState_RoundOver,
}
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	/*CreateNative("FF2_GetBossMaxSpeed", Native_GetBossMaxSpeed);
	CreateNative("FF2_SetBossMaxSpeed", Native_SetBossMaxSpeed);*/
...
public OnPluginStart()
{
	...
	cvarCircuitStun=CreateConVar("ff2_circuit_stun", "4", "Amount of seconds the Short Circuit stuns the boss for.  0 to disable", FCVAR_PLUGIN, true, 0.0);
	...
	HookEvent("teamplay_round_start", OnRoundCountdown, EventHookMode_Pre);
	...
	RegConsoleCmd("FF2 Class Info", Command_HelpPanelClass);
...
public OnConfigsExecuted()
{
	...
	// Used only in case FF2 is reloaded unexpectedly!
	...
	for(new client=1;client<=MaxClients;client++)
	{
		...
		if(clientcount)	
		{
			LogMessage("Freak Fortress 2 v%s was reloaded unexpectedly!", PLUGIN_VERSION);
			AttachStuff(client);
			ForceTeamWin(0); 
			CPrintToChat(client, "{olive}[FF2]{default} Freak Fortress 2 v%s has been unexpectedly reloaded! Ending current round!", PLUGIN_VERSION);
			clientcount=0;
		}
	}
}
...
public Action:OnRoundCountdown(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		LogError(GetRandomInt(1,2)==2 ? "[FF2] FROG FROG FROG FROG FROG" : "[FF2] frog frog frog frog frog");
		return Plugin_Continue;
	}
	...
	/*BossSpeed[0]=KvGetFloat(BossKV[Special[0]], "maxspeed", 340.0);
	if(BossSpeed[0]<=0)
	{
		new String:bossName[64];
		KvGetString(BossKV[Special[0]], "name", bossName, sizeof(bossName));
		LogError("[FF2 Bosses] Warning: Boss %s's max speed is below 0, setting to 340", bossName);
		BossSpeed[0]=340.0;
	}*/
	...
	if(BossLivesMax[0]<=0)
	{
		new String:bossName[64];
		KvGetString(BossKV[Special[0]], "name", bossName, sizeof(bossName));
		LogError("[FF2 Bosses] Warning: Boss %s has an invalid amount of lives, setting to 1", bossName);
		BossLivesMax[0]=1;
	}
	HasCompanions = false;
	...
	{	if (client>0 && IsValidEntity(client) && IsClientConnected(client))
		{
			...
			else if (ClientCookie[client] == TOGGLE_UNDEF || ClientCookie[client] == 0)
			{
			    	decl String:nick[64]; 
			    	GetClientName(client, nick, sizeof(nick));
...
{
	if(CheckRoundState()!=FF2RoundState_RoundActive)
	{
		return Plugin_Continue;
	}
...
{
	MusicTimer=INVALID_HANDLE;
	if(Enabled && CheckRoundState()==FF2RoundState_RoundActive)
	{
		KvRewind(BossKV[Special[0]]);
		if(KvJumpToKey(BossKV[Special[0]], "sound_bgm"))
		{
...
{
	if(IsValidClient(Boss[client]) && IsPlayerAlive(Boss[client]) && CheckRoundState()!=FF2RoundState_RoundOver)
	{
...
public Action:MakeBoss(Handle:timer, any:boss)
{
	...
	if(GetClientTeam(client)==OtherTeam) // No Living Spectators Pls
	{
	...
	/*if(!CheckRoundState())
	{
		BossHealthMax[client]=ParseFormula(client);
		...
		Debug("%N (client index %i, boss index %i) has %i lives", Boss[client], Boss[client], client, BossLivesMax[client]);
	}*/
	return Plugin_Continue;
}
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	switch(iItemDefinitionIndex)
	{
		...
		case 36: // Blutsauger
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "16 ; 1 ; 17 ; 0.01 ; 129 ; -2 ; 144 ; 1", true);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 142: // Gunslinger
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "286 ; 2.0 ; 344 ; 2.0", false);
				//286: 2x Max Building Health
				//344: 2x Sentry Range
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 154: // Pain Train
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.25 ; 149 ; 5.0 ; 204 ; 1", false);
				//2: +25% damage
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 208 ; 1", false);
				//204: On-miss; Hit yourself, idiot
		...
		case 405, 608, 133:  //Ali Baba's Wee Booties, Bootlegger, Gunboats
		{
	...
	{
		new Handle:itemOverride=PrepareItemHandle(item, _, _, "170 ; 1.25", false); 
		if(itemOverride!=INVALID_HANDLE)
		{
			item=itemOverride;
			return Plugin_Changed;
...
{
	new client=GetClientOfUserId(userid);
	if(!IsValidClient(client) || !IsPlayerAlive(client) || CheckRoundState()==FF2RoundState_RoundOver || IsBoss(client) || (FF2flags[client] & FF2FLAG_ALLOWSPAWNINBOSSTEAM))
	{
		return Plugin_Continue;
	}
...
public Action:Timer_Uber(Handle:timer, any:medigunid)
{
	...
	if(medigun && IsValidEntity(medigun) && CheckRoundState()==FF2RoundState_RoundActive)
	{
...
{
	if(!IsValidClient(client) || !Enabled || CheckRoundState()!=FF2RoundState_RoundActive)
	{
		return Plugin_Continue;
	}
...
public Action:Command_LoadCharset(client, args)
{
	...
	{
		CReplyToCommand(client, "{olive}[FF2]{default} You can only switch charsets at the end of a round!");
		return Plugin_Handled;
	}
	...
	for(new i; ; i++)
	{
		...
		if(StrContains(config, charset, false)>=0)
		{
			CReplyToCommand(client, "{olive}[FF2]{default} Charset has been switched to %s", config);
			FF2CharSet=i;
			FindCharacters();
			strcopy(FF2CharSetString, 2, "");
			break;
...
public Action:Command_ReloadCharset(client, args)
{
	if(!CheckRoundState() || CheckRoundState()==1)
	{
		CReplyToCommand(client, "{olive}[FF2]{default} You can only reload a charset at the end of a round!");
		return Plugin_Handled;
	}
	CReplyToCommand(client, "{olive}[FF2]{default} Current charset has been reloaded!");
	FindCharacters();
	return Plugin_Handled;
}
...
public OnClientDisconnect(client)
{
	if(Enabled)
	{
		...
		if(IsClientInGame(client) && IsPlayerAlive(client) && CheckRoundState()==FF2RoundState_RoundActive)
		{
...
public OnGameFrame() // Moving some stuff here and there
{
	...
	if(DeadRunMode && CheckRoundState()==FF2RoundState_RoundActive)
	{
...
public FF2_Tick(Float:gameTime)
{
	...
	{
		if((Enabled || Enabled2) && CheckRoundState()==FF2RoundState_PreGame)
		{
			SetControlPoint(true);
			if(checkDoors)
			{
	...
	if(gameTime >= CheckDoorsAt)
	{
		...
		if((!Enabled && CheckRoundState()!=FF2RoundState_PreGame) || (Enabled && CheckRoundState()!=FF2RoundState_RoundActive))
		{
			CheckDoorsAt = INACTIVE;
			return;
		}
	...
	{
		if(BossHealth[0]<countdownHealth || CheckRoundState()!=FF2RoundState_RoundActive || AliveMercPlayers>countdownPlayers)
		{
			executed2=false;
			DrawGameTimerAt = INACTIVE;
			return;
...
public FF2_RoundTick(Float:gameTime)
{
	...
	{
		if(!Enabled || CheckRoundState()==FF2RoundState_RoundOver || CheckRoundState()==FF2RoundState_PreGame)
		{
			FF2ClientTick = INACTIVE;
			return;
		}
...
stock OnlyScoutsLeft()
{
	...
	for(new client; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && IsPlayerAlive(client) && GetClientTeam(client)==OtherTeam)
		{
...
public TF2_OnConditionAdded(client, TFCond:condition)
{
	if(Enabled && IsBoss(client) && (condition==TFCond_Jarated || condition==TFCond_MarkedForDeath || (condition==TFCond_Dazed && TF2_IsPlayerInCondition(client, TFCond:42))))
	{
		...
		{
			BossCharge[boss][0]-=25.0;  //TODO: Allow this to be customizable
...
{
	if(!Enabled || !IsPlayerAlive(client) || CheckRoundState()!=FF2RoundState_RoundActive || !IsBoss(client))
	{
		return Plugin_Continue;
	}
...
{
	if(!Enabled || !IsPlayerAlive(client) || CheckRoundState()!=FF2RoundState_RoundActive || !IsBoss(client) || args!=2)
	{
		return Plugin_Continue;
	}
...
public Action:OnSuicide(client, const String:command[], args)
{
	...
	if(Enabled && IsBoss(client) && (canBossSuicide ? !CheckRoundState() : true) && CheckRoundState()!=FF2RoundState_RoundOver)
	{
...
public Action:OnJoinTeam(client, const String:command[], args)
{
	...
	if(CheckRoundState()!=FF2RoundState_RoundActive && !IsBoss(client) || !IsPlayerAlive(client))  //No point in showing the VGUI if they can't change teams
	{
		switch(team)
		{
			case TFTeam_Red:
...
public Action:OnRPS(Handle:event, const String:eventName[], bool:dontBroadcast)
{
	...
	if(!IsBoss(winner) && IsBoss(loser)) // Boss Loses on RPS? Kill current boss.
	{
		RPSWinner=winner;
		RPSLoser[loser]=true;
		Damage[winner]+=FF2_GetBossHealth(FF2_GetBossIndex(loser));
			...
			if(attacker<=MaxClients)
			{
				...
				switch(index)
				{
					case 14, 201, 230, 402, 526, 664, 752, 792, 801, 851, 881, 890, 899, 908, 957, 966:  //Sniper Rifle, Strange Sniper Rifle, Sydney Sleeper, Bazaar Bargain, Machina, Festive Sniper Rifle, Hitman's Heatmaker, Botkiller Sniper Rifles
					{
						...
						if(index==752 && CheckRoundState()!=FF2RoundState_RoundOver)  //Hitman's Heatmaker
						{
							new Float:chargelevel=(IsValidEntity(weapon) && weapon>MaxClients ? GetEntPropFloat(weapon, Prop_Send, "m_flChargedDamage") : 0.0);
			...
			else
			{
				...
				if(GetEdictClassname(attacker, classname, sizeof(classname)) && !strcmp(classname, "trigger_hurt", false))
				{
					if (bSpawnTeleOnTriggerHurt)
					{
						...
						if(IsBoss(client))
							TeleportToMultiMapSpawn(client, RoundTick>=30 ? BossTeam : 0);
					}
...
{
	if(Enabled && IsBoss(client) && CheckRoundState()==FF2RoundState_RoundActive && !TF2_IsPlayerCritBuffed(client) && !BossCrits)
	{
		result=false;
		return Plugin_Changed;
	}
	else if (Enabled && !IsBoss(client) && CheckRoundState()==FF2RoundState_RoundActive && IsValidEntity(weapon))
	{
...
stock ParseFormula(client)
{
	...
	if(formula[0]!='\0')
	{
		if(!KvGetNum(BossKV[Special[client]], "health_formula_mode", 0) && playing<5)
		{
			playing+=2;
		}
	}
...
FindCompanion(boss, players, bool:omit[])
{
	...
	if(playersNeeded<players && strlen(companionName))  //Only continue if we have enough players and if the boss has a companion
	{
		...
		CreateTimer(0.1, MakeBoss, GetBossIndex(companion));
		...
		if(PickCharacter(boss, companion))  //TODO: This is a bit misleading
		{
			...
			/*BossSpeed[companion]=KvGetFloat(BossKV[Special[companion]], "maxspeed", 340.0);
			if(BossSpeed[companion]<=0)
			{
				new String:bossName[64];
				KvGetString(BossKV[Special[companion]], "name", bossName, sizeof(bossName));
				LogError("[FF2 Bosses] Warning: Boss %s's max speed is below 0, setting to 340", bossName);
				BossSpeed[companion]=340.0;
			}*/
...
/*public Native_GetBossMaxSpeed(Handle:plugin, numParams)
{
	return _:BossSpeed[Float:GetNativeCell(1)];
}

public Native_SetBossMaxSpeed(Handle:plugin, numParams)
{
	BossSpeed[GetNativeCell(1)]=Float:GetNativeCell(2);
}*/

public Native_GetRoundState(Handle:plugin, numParams)
{
	if(CheckRoundState()<=FF2RoundState_PreRound)
	{
		return FF2RoundState_PreRound;
	}
...
public CheckRoundState()
{
	switch(GameRules_GetRoundState())
	{
		...
		{
			return FF2RoundState_RoundActive;
		}
		default:
		{
			return FF2RoundState_RoundOver;
		}
	}
...
UpdateHealthBar()
{
	if(!Enabled || !GetConVarBool(cvarHealthBar) || g_Monoculus!=-1 || CheckRoundState()==FF2RoundState_PreGame)
	{
		return;
	}
