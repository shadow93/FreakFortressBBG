...
{
	OnMapEnd();
	if (!ReloadFF2 && CheckRoundState() == FF2RoundState_RoundActive)
	{
...
public Action TF2Items_OnGiveNamedItem(int client, char[] classname,int iItemDefinitionIndex, Handle &item)
{
	...
	switch(IsBoss(client))
	{
		case true: 	// Block these from being equipped on bosses
		{
			if(!StrContains(classname, "tf_wearable")|| !StrContains(classname, "tf_powerup_bottle") || !StrContains(classname, "tf_usableitem"))
			{
				return Plugin_Stop;
			}
			return Plugin_Continue;
		}
		case false: // Item Overrides for mercs
		{
			...
			// Thanks valve, for making weapons complete clusterfucks to deal with since the Gun Mettle update...as if breaking plugins wasn't already enough.
...
public Action Command_ReloadFF2(client, args)
{
    ...
    {
        case FF2RoundState_Loading, FF2RoundState_RoundOver:
        {
...
public FF2_Tick(Float:gameTime)
{
	...
	if(gameTime >= CheckDoorsAt)
	{
		...
		if((!Enabled && CheckRoundState()!=FF2RoundState_Loading) || (Enabled && CheckRoundState()!=FF2RoundState_RoundActive))
		{
			CheckDoorsAt = INACTIVE;
			return;
		}
...
public FF2_RoundTick(Float:gameTime)
{
	...
	{
		if(!Enabled || CheckRoundState()==FF2RoundState_RoundOver || CheckRoundState()==FF2RoundState_Loading)
		{
			FF2ClientTick = INACTIVE;
			return;
		}
		...
		for(new client=1; client<=MaxClients; client++)
		{
			if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
			{
				...
				if(validwep && class==TFClass_Sniper && weapon==GetPlayerWeaponSlot(client, TFWeaponSlot_Primary))
				{
					addthecrit=true;
				}
	...
	else
	{
		...
		if(boss!=-1)
		{
			if(attacker<=MaxClients)
			{
				...
				switch(index)
				{
					case 230, 402, 526, 752, 1098:  // Sydney Sleeper, Bazaar Bargain, Machina, Hitman's Heatmaker, The Classic
					{
						...
						/*if(index==1098 && CheckRoundState()!=FF2RoundState_RoundOver) // The Classic
						{
							new Float:chargelevel=(IsValidEntity(weapon) && weapon>MaxClients ? GetEntPropFloat(weapon, Prop_Send, "m_flChargedDamage") : 0.0);
							if(chargelevel>=150.0 && (!TF2_IsPlayerInCondition(client, TFCond_Dazed) || !TF2_IsPlayerInCondition(attacker, TFCond_MarkedForDeath)))
							{
								TF2_StunPlayer(client, circuitStun, 0.0, TF_STUNFLAGS_SMALLBONK, attacker);
								TF2_AddCondition(attacker, TFCond_MarkedForDeath, circuitStun*2);
								EmitSoundToAll(AHH_PISS, attacker);
							}
						}*/
			...
			else
			{
				...
				{
					if (bSpawnTeleOnTriggerHurt && IsBoss(client) && CheckRoundState()==FF2RoundState_RoundActive)
					{
...
stock ParseFormula(client)
{
	...
	if(formula[0]!='\0')
	{
		if(KvGetNum(BossKV[Special[client]], "subversion")<31 && playing<5)
		{
			playing+=2;
		}
	}
