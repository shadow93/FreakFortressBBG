...
public Action:Event_Setup(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    for(new boss; boss<=MaxClients; boss++)
    {
        ...
        if(Boss[boss])
        {
            ...
            BossInfoTimer[boss][0]=CreateTimer(10.0, BossInfoTimer_Begin, boss, TIMER_FLAG_NO_MAPCHANGE);
        }
    }
...
public Action:BossInfoTimer_Begin(Handle:timer, any:client)
{
    ...
    Debug("BOSS INFO TIMER BEGIN");
    BossInfoTimer[client][1]=CreateTimer(30.0, BossInfoTimer_ShowInfo, client, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
    return Plugin_Continue;
}

public Action:BossInfoTimer_ShowInfo(Handle:timer, any:client)
{
    Debug("BOSS INFO TIMER SHOWING INFO");
    ...
    for(int abilityIdx=0; ;abilityIdx++)
    {
        char abilityNum[32];
        Format(abilityNum, sizeof(abilityNum), "ability%i", abilityIdx);
        ...
        if(KvJumpToKey(BossKV[characterIdx[client]], abilityNum))
        {
            new String:plugin_name[64];
            KvGetString(BossKV[characterIdx[client]], "plugin_name", plugin_name, 64);
            new buttonIdx=KvGetNum(BossKV[characterIdx[client]], "buttonmode", 0);
            if(buttonIdx)
            {
                showtip[buttonIdx]=true;
                break;
            }
        }
        else
    ...
    bool showToolTip[4]=false;
    showToolTip[1]=showtip[1]; //&& CheckInfoCookies(Boss[client], 0);
    showToolTip[2]=showtip[2]; //&& CheckInfoCookies(Boss[client], 1);
    showToolTip[3]=showtip[3]; //&& CheckInfoCookies(Boss[client], 2);

    if(showToolTip[1])
    {
        CreateAttachedAnnotation(Boss[client], Boss[client], true, 5.0, "Right Mouse Button Buttonmode");
    }
    else if(showToolTip[2])
    {
        CreateAttachedAnnotation(Boss[client], Boss[client], true, 5.0, "%t", "Reload Buttonmode");
    }
    else if(showToolTip[3])
    {
        CreateAttachedAnnotation(Boss[client], Boss[client], true, 5.0, "%t", "Special Buttonmode");
    }    
    else
    {
        BossInfoTimer[client][1]=INVALID_HANDLE;
...
stock ParseFormula(boss, const String:key[], defaultValue)
{
    ...
    if(StrEqual(key, "health", false) || StrEqual(key, "rage_damage", false) || StrEqual(key, "ragedamage", false))
    {
        result*=10;
    }
