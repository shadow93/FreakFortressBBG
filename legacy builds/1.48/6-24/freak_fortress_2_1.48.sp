...
stock FindVersionData(Handle:panel, versionIndex)
{
    switch(versionIndex)
    {
        case 121: // 1.48
        {
            DrawPanelText(panel, "1) [Commands] Added 'ff2_bot_{enable|disable}' to toggle bot logic on/off (SHADoW)");
            DrawPanelText(panel, "2) [Commands] Added 'ff2_generate_navmesh' to generate .nav files for maps lacking one(SHADoW)");        
            DrawPanelText(panel, "3) [Gameplay] Implemented bot logic (SHADoW/tRololo312312)");
            DrawPanelText(panel, "4) [Gameplay] A single bot will spawn if human players connected are below 2 (SHADoW)");
        }
        case 120: // 1.47
        {
...
PlayBGM(client, String:music[], Float:time, bool:loop=true, char[] name="", char[] artist="")
{
   ...
   //Call_PushStringEx(temp[2], sizeof(temp[]), SM_PARAM_STRING_UTF8 | SM_PARAM_STRING_COPY, SM_PARAM_COPYBACK);
    ...
    if(!name[0])
    {
        Format(name[0], 256, "Unknown Song");
    }
    
    if(!artist[0])
    {
        Format(artist[0], 256, "Unknown Artist");
    }
...
public Action Command_Tracklist(int client, int args)
{
    ...
    if(KvJumpToKey(BossKV[characterIdx[0]], "sound_bgm"))
    {
        ...
        for(new trackIdx=1;trackIdx<=index-1;trackIdx++)
        {
            ...
            KvGetString(BossKV[characterIdx[0]], id3[1], id3[3], sizeof(id3[]));
            Format(id3[4], sizeof(id3[]), "%s - %s", id3[3], id3[2]);
...
public Command_TrackListH(Handle:menu, MenuAction:action, param1, param2)
{
    switch(action)
    {
        case MenuAction_End:
...
StopMusic(client=0, bool:endloop=false, bool:permanent=false)
{
