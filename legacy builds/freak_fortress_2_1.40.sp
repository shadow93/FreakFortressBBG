...
new bool:minimalHUD[MAXPLAYERS+1]=false;
...
public Action:BossInfoTimer_ShowInfo(Handle:timer, any:client)
{
    ...
    for(new n=1; ; n++)
    {
        ...
        if(client==-1 || characterIdx[client]==-1 || !BossKV[characterIdx[client]])
        {
            return Plugin_Stop;
        }

        KvRewind(BossKV[characterIdx[client]]);
        if(KvJumpToKey(BossKV[characterIdx[client]], s))
        {
            new String:plugin_name[64];
            KvGetString(BossKV[characterIdx[client]], "plugin_name", plugin_name, 64);
            if(KvGetNum(BossKV[characterIdx[client]], "buttonmode", 0)==2)
            {
                see=true;
                break;
            }
    ...
    {
        SetHudTextParams(0.75, 0.7, 0.15, 255, 255, 255, 255, makeScroll ? 2 : 0);
...
public Action:DifficultyMenu(client, args)
{
    if (IsValidClient(client))
    {    
        if(IsBoss(client) && CheckRoundState()==FF2RoundState_RoundRunning)
        {
        ...
        case MenuAction_Select:
        {
            switch(param2)
            {
                case 0: 
                {
                    ...
                    if(!IsBoss(param1) || IsBoss(param1) && CheckRoundState()!=FF2RoundState_RoundRunning)
                    {
                        DifficultyMenu(param1, 0);
                    }
                    return;
...
stock ShowFF2Text(client, const String:buffer[], any:...)
{
    ...
    {
        ReplaceString(message, sizeof(message), "\n", "");  //Get rid of newlines && timeleft>30
        CreateTFStypeMessage(client, message, (DeadRunMode == true ? "ico_ghost" : (DrawGameTimerAt!=INACTIVE) ? ((timeleft>=10 && timeleft<30) ? "ico_notify_thirty_seconds" : (timeleft<10) ? "ico_notify_ten_seconds" : "ico_notify_sixty_seconds") : roundOvertime ? "ico_notify_flag_moving_alt" : "leaderboard_streak"), 0);
    }
    else
    {
...
stock ShowGameText(client, const String:buffer[], any:...)
{
    ...
    {
        ReplaceString(message, sizeof(message), "\n", "");  //Get rid of newlines
        CreateTFStypeMessage(client, message);
    }
    else
    {
...
stock ShowGameTextAll(String:buffer[], any:...)
{
    decl String:message[512];
    for(new client=1;client<=MaxClients;client++)
    {
...
public FF2_Tick(Float:gameTime)
{
    ...
    {    
        ...
        else if(!PointType && LivingMercs<=AliveToEnable && !executed)
        {
            if(LivingMercs>1)
            {
                ShowGameTextAll("%t", "point_enable", AliveToEnable);
            }
            else
            {
                PrintHintTextToAll("%t", "point_enable", AliveToEnable);
            }
            if(LivingMercs<=AliveToEnable)
            {
                char sound[64];
                if(GetRandomInt(0, 1))
                {
        ...
        if(BossHealth[boss]-damage<=BossHealthMax[boss]*lives)
        {
            ...
            for(new target=1; target<=MaxClients; target++)
            {
                if(IsValidClient(target) && !(FF2Flags[target] & FF2FLAG_HUDDISABLED))
                {
                    ...
                    {
                        ShowGameText(target, "%t", ability, bossName, BossLives[boss]);
                    }
                    else
                    {
            ...
            if(attacker<=MaxClients)
            {
                ...
                if(bIsTelefrag)
                {
                    ...
                    if(IsValidClient(teleowner) && teleowner!=attacker)
                    {
                        ...
                        if(!(FF2Flags[teleowner] & FF2FLAG_HUDDISABLED))
                        {
                            ...
                            if(LivingMercs>1)
                            {
                                ShowGameText(teleowner, "%t", "Telefrag Assist", attacker, spcl);
                            }
                            else
                            {
                                PrintHintText(teleowner, "%t", "Telefrag Assist", attacker, spcl);
                            }
                        }
                    }
                    ...
                    if(!(FF2Flags[attacker] & FF2FLAG_HUDDISABLED))
                    {
                        ...
                        if(LivingMercs>1)
                        {
                            ShowGameText(attacker, "%t", "Telefrag", spcl);
                        }
                        else
                        {
                            PrintHintText(attacker, "%t", "Telefrag", spcl);
                        }
                    }
                    ...
                    if(!(FF2Flags[client] & FF2FLAG_HUDDISABLED))
                    {
                        if(LivingMercs>1)
                        {
                            ShowGameText(client, "%t", "Telefragged", attacker);
                        }
                        else
                        {
                            PrintHintText(client, "%t", "Telefragged", attacker);
                        }
                    }
                    return Plugin_Changed;
                }
                ...
                switch(index)
                {
                    ...
                    case 307, 416:   // Chdata's Market Gardener backstab + VoIDeD's Caber backstab
                    {
                        if (RemoveCond(attacker, TFCond_BlastJumping)) // New way to check explosive jumping status
                        {
                            ...
                            if(LivingMercs>1)
                            {
                                ShowGameText(attacker, "%t", index == 416 ? "Market Gardener" : "Ullapool Caber", spcl);  //You just market-gardened the boss!
                                ShowGameText(client, "%t", index == 416 ? "Market Gardened" : "Ullapool Cabered");  //You just got market-gardened!
                ...
                if(bIsBackstab)
                {
                    ...
                    if(!(FF2Flags[attacker] & FF2FLAG_HUDDISABLED))
                    {
                        ...
                        if(LivingMercs>1)
                        {
                            ShowGameText(attacker, "%t", "Backstab", spcl);
                        }
                        else
                        {
                    ...
                    {    
                        if(LivingMercs>1)
                        {
                            ShowGameText(client, "%t", "Backstabbed");
                        }
                        else
                        {
        ...
        else
        {
            ...
            if(index==307)  //Ullapool Caber
            {
                if(detonations[attacker]<allowedDetonations && !RemoveCond(attacker, TFCond_BlastJumping)) // Ignore if caber 'backstabbing'
                {
                    ...
                    if(LivingMercs>1)
                    {
                        ShowGameText(attacker, "%t", "Detonations Left", allowedDetonations-detonations[attacker]);
                    }
                    else
                    {
...
public Action:OnStomp(attacker, victim, &Float:damageMultiplier, &Float:damageBonus, &Float:JumpPower)
{
    ...
    if(IsBoss(attacker))
    {
        ...
        if(LivingMercs>1)
        {
            ShowGameText(attacker, "%t", "Boss Goomba Stomps", victim);
            ShowGameText(victim, "%t","Goomba Stomped Player", spcl);
        }
        else
        {
    ...
    else if(IsBoss(victim))
    {
        ...
        if(LivingMercs>1)
        {
            ShowGameText(attacker, "%t", "Player Goomba Stomps", spcl, GoombaCount[attacker]);
            ShowGameText(victim, "%t", "Goomba Stomped Boss", GoombaCount[attacker], attacker);
        }
        else
        {
...
FindCompanion(boss, players, bool:omit[])
{
    ...
    Debug("Companions: %i of %i", Companions, TotalCompanions);
...
UseAbility(boss, const String:plugin_name[], const String:ability_name[], slot, buttonMode=0)
{
    new bool:enabled=true;
