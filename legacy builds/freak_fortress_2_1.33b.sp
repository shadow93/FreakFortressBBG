...
#define FORK_MINOR_REVISION "33"
#define FORK_SUB_REVISION	"a"
...
stock FindVersionData(Handle:panel, versionIndex)
{
	switch(versionIndex)
	{
		case 96: // 1.33
		{
			DrawPanelText(panel, "1) Fixed living spectator bugs...again - see #11 for details (SHADoW)");		
			DrawPanelText(panel, "2) TF2_RegeneratePlayer can now be used to regenerate a boss's weapon loadout - see #11 for details (SHADoW)");
			DrawPanelText(panel, "3) Fixed 'companion' section being case-sensitive (SHADoW)");
			DrawPanelText(panel, "4) Added 'preset' bool to FF2_OnSpecialSelected (Wliu)");
			DrawPanelText(panel, "5) Market Gardener & Ullapool Caber can only crit while blast jumping (SHADoW)");
		}
		case 95: // 1.33
		{
			DrawPanelText(panel, "6) Minions now have the same minicrit / crit boost restrictions (SHADoW)");
			...
			DrawPanelText(panel, "9) Fixed 'bossteam' option endlessly switching teams if setting is 1 (SHADoW)");
			DrawPanelText(panel, "10) Lowered huntsman skewer taunt cooldown from 10 seconds to 7 (SHADoW)");
		}	
		case 94: // 1.33
		{
			...
			DrawPanelText(panel, "13) New cvar - 'ff2_countdown_overtime' - determines if #12 would fire (SHADoW)");
...
stock bool:IsFF2Map()
{
	...
	BuildPath(Path_SM, config, sizeof(config), "data/freak_fortress_2/maps.cfg");
	if(!FileExists(config))
	{
...
stock bool:CheckToChangeMapDoors()
{
	...
	new String:config[PLATFORM_MAX_PATH];
	checkDoors=false;
	BuildPath(Path_SM, config, sizeof(config), "data/freak_fortress_2/doors.cfg");
	if(!FileExists(config))
	{
...
CheckToTeleportToSpawn()
{
    ...
    BuildPath(Path_SM, config, sizeof(config), "data/freak_fortress_2/spawn_teleport.cfg");
...
public Action:MakeModelTimer(Handle:timer, any:client)
{
	if(IsValidClient(Boss[client]) && IsPlayerAlive(Boss[client]) && CheckRoundState()!=FF2RoundState_RoundEnd)
	{
		...
		{
			SetVariantString("");
			LogError("[FF2 Bosses] Character %s is missing file '%s'! Using default model!", bossName, model);
		}
...
public Action TF2Items_OnGiveNamedItem(int client, char[] classname,int iItemDefinitionIndex, Handle &item)
{
	...
	switch(iItemDefinitionIndex)
	{
		...
		case 36: // Blutsauger
		{
			Handle itemOverride=PrepareItemHandle(item, _, _, "16 ; 1 ; 17 ; 0.01 ; 144 ; 1", true);
			if(itemOverride!=null)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		{
			Handle itemOverride;
			itemOverride=PrepareItemHandle(item, _, _, "58 ; 3.2 ; 339 ; 1 ; 340 ; 1", false);
				//58: 220% self damage force
			if(itemOverride!=null)
			{
				item=itemOverride;
		...
		{ 
			if(!DeadRunMode)
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.25 ; 125 ; -25 ; 109 ; 0.5 ; 394 ; 0.85 ; 418 ; 1 ; 419 ; 100 ; 532 ; 0.5 ; 651 ; 0.5 ; 709 ; 1", true); 
					//2: +25% damage bonus 
					//125: -25 max health 
	...
	else
	{
		...
		if(boss!=-1)
		{
			...
			else
			{
				...
				{
					if (bSpawnTeleOnTriggerHurt && IsBoss(client) && CheckRoundState()==FF2RoundState_RoundRunning && damage>=400.0)
					{
