...
public OnMapStart()
{
	...
	if (!StrContains(chkcurrentmap,"vsh_dr_") || !StrContains(chkcurrentmap,"dr_") || !StrContains(chkcurrentmap,"deadrun_"))
	{
		DeadRunMode = true;
...
public EnableFF2()
{
	...
	if(steamtools)
	{
		...
		if(!DeadRunMode)
			...
			Format(gameDesc, sizeof(gameDesc), "FF2 Deathrun (%s)", PLUGIN_VERSION);			
...
ShowGameText(const String:strMessage[]) 
{
	...
	if(DeadRunMode)
		DispatchKeyValue(iEntity,"icon", "ico_ghost");
	else
...
EquipBoss(boss)
{
	...
	for(new i=1; ; i++)
	{
		...
		if(KvJumpToKey(BossKV[Special[boss]], weapon))
		{
			...
			if(attributes[0]!='\0')
			{
				Format(attributes, sizeof(attributes), "2 ; 3.1 ; %s", attributes);
					//2: x3.1 damage
			}
			else
			{
				attributes="2 ; 3.1 ; 2025 ; 2 ; 2014 ; 1";
					//2: x3.1 damage
			...
			#if defined _tf2attributes_included
			{
				...
				if(TF2_GetPlayerClass(client) == TFClass_Scout)
					TF2Attrib_SetByDefIndex(client, 68, 1.0);  // +1 x cap rate
				else
					TF2Attrib_SetByDefIndex(client, 68, 2.0);  // +2 x cap rate
			}
			#endif
	...
	if(DeadRunMode)
	{
		for(new i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i) && IsPlayerAlive(i))
			{
				...
				else if(GetClientTeam(i) == BossTeam)
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	if(((!StrContains(classname,"tf_weapon_rocketlauncher") || !StrContains(classname,"tf_weapon_particle_cannon") || !StrContains(classname,"tf_weapon_pipebomblauncher"))) && DeadRunMode)
	{
		switch(iItemDefinitionIndex)
		{
			case 237, 265: // Rocket & Sticky Jumper
			{
				new Handle:itemOverride=PrepareItemHandle(item, _, _, "59 ; 0.5", true); 
				if(itemOverride!=INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			default: // All others
			{
				new Handle:itemOverride=PrepareItemHandle(item, _, _, "59 ; 0.5", false); 
				if(itemOverride!=INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
...
public Action:CheckItems(Handle:timer, any:client)
{
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			...
			case 304: // Amputator (Lags players)
			{
				if(DeadRunMode)
				{
					TF2_RemoveWeaponSlot(client, TFWeaponSlot_Melee);
					weapon=SpawnWeapon(client, "tf_weapon_bonesaw", 8, 1, 0, "");
				}
			}
	...
	for(new client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			else if(class==TFClass_Spy)
			{
				...
				if(weapon==GetPlayerWeaponSlot(client, 4))
				{
...
public Action:OnCallForMedic(client, const String:command[], args)
{
	...
	if(RoundFloat(BossCharge[boss][0])==100)
	{
		new String:ability[10], String:lives[MAXRANDOMS][3];
		for(new i=1; i<MAXRANDOMS; i++)
		{
			Format(ability, sizeof(ability), "ability%i", i);
			KvRewind(BossKV[Special[boss]]);
	...
	for(new lives=1; lives<BossLives[boss]; lives++)
	{
		if(BossHealth[boss]-damage<=BossHealthMax[boss]*lives)
		{
			...
			for(new target=1; target<=MaxClients; target++)
			{
				...
				{
					Format(ability, sizeof(ability), "%t", ability, bossName, BossLives[boss]);
					ShowGameText(ability);
				}
			}
