...
EquipBoss(boss)
{
	...
	for(new i=1; ; i++)
	{
		...
		if(KvJumpToKey(BossKV[Special[boss]], weapon))
		{
			...
			if(attributes[0]!='\0')
			{
				Format(attributes, sizeof(attributes), (DeadRunMode == true ? "703 ; 1 ; %s" : "2 ; 3.1 ; %s"), attributes);
					//2: x3.1 damage
			}
			else
			{
				attributes=(DeadRunMode == true ? "2025 ; 2 ; 2014; 1" : "2 ; 3.1 ; 2025 ; 2 ; 2014 ; 1");
					//2: x3.1 damage
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	if(((!StrContains(classname,"tf_weapon_rocketlauncher") || !StrContains(classname,"tf_weapon_particle_cannon") || !StrContains(classname,"tf_weapon_pipebomblauncher"))) && DeadRunMode)
	{
		switch(iItemDefinitionIndex)
		{
			case 237, 265: // Rocket & Sticky Jumper
			{
				new Handle:itemOverride=PrepareItemHandle(item, _, _, "59 ; 0.3", true); 
				if(itemOverride!=INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			default: // All others
			{
				new Handle:itemOverride=PrepareItemHandle(item, _, _, "59 ; 0.3", false); 
				if(itemOverride!=INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
