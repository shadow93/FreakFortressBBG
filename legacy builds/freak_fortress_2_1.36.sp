...
EquipBoss(boss)
{
    ...
    for(new i=1; ; i++)
    {
        ...
        if(KvJumpToKey(BossKV[Special[boss]], weapon))
        {
            ...
            new BossWeapon=SpawnWeapon(client, weapon, wepIdx, 101, 5, attributes);
...
public void Timers_PreThink(int client, float gTime)
{
    ...
    if(gTime>=KillRPSLosingBossAt[client])
    {
        if(IsPlayerAlive(client))
        {
            SDKHooks_TakeDamage(client, RPSWinner, RPSWinner, float(BossHealth[Boss[client]]), DMG_GENERIC, -1);
        }
        KillRPSLosingBossAt[client]=INACTIVE;
    }
    ...
    if(gTime>=PrepareMercAt[client])
    {
        ...
        if(GetClientTeam(client)!=MercTeam)
        {
            ...
            AssignTeam(client, TFTeam:MercTeam, GetRandomInt(1,9));
    ...
    if(gTime>=InspectPlayerInventoryAt[client])
    {
        ...
        for(int wepSlot=0; wepSlot<=5; wepSlot++)
        {
            ...
            if(weaponEntId && IsValidEdict(weaponEntId))
            {
                ...
                switch(weaponIdx)
                {
                    ...
                    case 589:  //Eureka Effect
                    {
                        ...
                        {    
                            ...
                            weaponEntId=SpawnWeapon(client, "tf_weapon_wrench", 7, 1, 0, "");
                        }
                    }
                }
                ...
                {
                    weaponEntId=SpawnWeapon(client, "tf_weapon_smg", 16, 1, 6, "149 ; 1.5 ; 15 ; 0.0 ; 1 ; 0.85");
                }
...
{
    if(IsValidClient(Boss[client], false))
    {
