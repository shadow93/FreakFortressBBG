	...
	for(new i=1; i<=dead && i<=totalMinions; i++)
	{
		...
		switch(weaponMode)
		{
			...
			case 1:
			{
				...
				weapon=SpawnWeapon(clone, classname, index, 101, 0, attributes, bool:FF2_GetAbilityArgument(boss, this_plugin_name, ability_name, 12));
...
Rage_Bow(boss, const String: ability_name[])
{
	new client=GetClientOfUserId(FF2_GetBossUserId(boss));
	TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
	new weapon=SpawnWeapon(client, "tf_weapon_compound_bow", 1005, 100, 5, "6 ; 0.5 ; 37 ; 0.0 ; 280 ; 19", bool:FF2_GetAbilityArgument(boss, this_plugin_name, ability_name, 1));
		...
		if(FF2_HasAbility(boss, this_plugin_name, "special_cbs_multimelee"))
		{
			if(GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon")==GetPlayerWeaponSlot(attacker, TFWeaponSlot_Melee))
			{
				...
				switch(GetRandomInt(0, 2))
				{
					case 0:
					{
						weapon=SpawnWeapon(attacker, "tf_weapon_club", 171, 101, 5, "68 ; 2 ; 2 ; 3.0", bool:FF2_GetAbilityArgument(boss, this_plugin_name, "special_cbs_multimelee", 1));
					}
					case 1:
					{
						weapon=SpawnWeapon(attacker, "tf_weapon_club", 193, 101, 5, "68 ; 2 ; 2 ; 3.0", bool:FF2_GetAbilityArgument(boss, this_plugin_name, "special_cbs_multimelee", 1));
					}
					case 2:
					{
						weapon=SpawnWeapon(attacker, "tf_weapon_club", 232, 101, 5, "68 ; 2 ; 2 ; 3.0", bool:FF2_GetAbilityArgument(boss, this_plugin_name, "special_cbs_multimelee", 1));
					}
				}
