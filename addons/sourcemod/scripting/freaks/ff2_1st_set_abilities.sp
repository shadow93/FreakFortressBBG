...
public Action:OnRoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	for(new client=1; client<=MaxClients; client++)
	{
		...
		if(IsClientInGame(client) && CloneOwnerIndex[client]!=-1 && GetClientTeam(client)==FF2_GetBossTeam())  //FIXME: IsClientInGame() shouldn't be needed
		{
			CloneOwnerIndex[client]=-1;
	...
	for(new i=1; i<=dead && i<=totalMinions; i++)
	{
		...
		switch(weaponMode)
		{
			...
			case 1:
			{
				...
				weapon=SpawnWeapon(clone, classname, index, 101, 0, attributes, bool:FF2_GetAbilityArgument(boss, this_plugin_name, "rage_cloneattack", 13));
...
Rage_Slowmo(boss, const String:ability_name[])
{
	...
	if(client)
	{
		CreateTimer(duration, Timer_RemoveEntity, EntIndexToEntRef(AttachParticle(client, FF2_GetBossTeam()==_:TFTeam_Blue ? "scout_dodge_blue" : "scout_dodge_red", 75.0)), TIMER_FLAG_NO_MAPCHANGE);
	}
...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(CloneOwnerIndex[client]!=-1 && GetClientTeam(client)==FF2_GetBossTeam())  //Switch clones back to the other team after they die
	{
		CloneOwnerIndex[client]=-1;
		FF2_SetFF2flags(client, FF2_GetFF2flags(client) & ~FF2FLAG_CLASSTIMERDISABLED);
